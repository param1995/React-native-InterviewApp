...
export default function ReviewSubmission({ route, navigation }) {
  ...
  async function saveReview() {
    ...
    if (!score.trim() || isNaN(numScore) || numScore < 0 || numScore > 10) {
      Alert.alert("Invalid Score", "Please enter a valid score between 0 and 10.");
      return;
    }

    if (!comments.trim()) {
      Alert.alert("Missing Comments", "Please provide review comments.");
      return;
    }
    ...
    try {
      ...
      all[idx] = {
        ...
        review: {
          ...
          reviewedAt: Date.now(),
          reviewerId: "reviewer@test.com" // In a real app, get from auth
        },
      };
  ...
  const renderAnswer = (answer, index) => {
    const questionText = interview?.questions?.[answer.qIndex] || `Question ${answer.qIndex + 1}`;
    const isPlaying = playingIndex === index;

    return (
      <View key={answer.id} style={[styles.answerCard, responsiveStyles.answerCard]}>
        <View style={styles.answerHeader}>
          ...
          <Text style={[styles.recordedTime, responsiveStyles.recordedTime]}>
            📅 {new Date(answer.recordedAt || submission.submittedAt).toLocaleString()}
          </Text>
        </View>
        ...
        <View style={styles.audioControls}>
          <TouchableOpacity
            ...
            onPress={() => isPlaying ? stopPlayback() : play(answer.uri, index)}
            activeOpacity={0.8}
          >
            <Text style={[
        ...
        >
          ...
          <View style={[styles.content, responsiveStyles.content]}>
            ...
            <View style={[styles.header, responsiveStyles.header]}>
              <Text style={[styles.submissionTitle, responsiveStyles.submissionTitle]}>
                📋 Review Submission
              </Text>
              <Text style={[styles.submissionId, responsiveStyles.submissionId]}>
            ...
            {/* Interview Info */}
            <View style={[styles.interviewInfo, responsiveStyles.interviewInfo]}>
              <Text style={[styles.interviewTitle, responsiveStyles.interviewTitle]}>
                {interview?.title || "Interview"}
              </Text>
              <Text style={[styles.interviewDesc, responsiveStyles.interviewDesc]}>
                {interview?.description || ""}
              </Text>
            </View>

            {/* Answers Section */}
            <View style={[styles.answersSection, responsiveStyles.answersSection]}>
              <Text style={[styles.sectionTitle, responsiveStyles.sectionTitle]}>
                Candidate Answers ({submission.answers?.length || 0})
              </Text>
              {submission.answers && submission.answers.length > 0 ? (
                ...
                <View style={[styles.noAnswers, responsiveStyles.noAnswers]}>
                  <Text style={[styles.noAnswersText, responsiveStyles.noAnswersText]}>
